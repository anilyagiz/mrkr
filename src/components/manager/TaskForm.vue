<template>
  <form @submit.prevent="onSubmit" class="bg-white dark:bg-dark-grey rounded-lg p-1">
    <div class="p-5 pr-3 flex flex-col gap-6 max-h-[90vh] overflow-y-scroll">
      ...
      <BaseInput ref="inputTitle" v-model="form.task.title" inputName="Başlık" placeholder="e.g. Başlık" />
      <BaseTextarea v-model="form.task.description" inputName="Açıklama"
        placeholder="e.g Görev Açıklaması" />
      <div class="flex flex-col gap-3">
        <p class="text-medium-grey text-xs font-bold">Alt İş Paketleri</p>
        <div ref="errorSubtasks" class="flex items-center justify-between gap-4"
          ...
          <BaseInput :ref="el => { inputs[index] = el }" v-model="subtask.title"
            :placeholder="subtaskPlaceholders[index] ? subtaskPlaceholders[index] : 'alt is paketi basligi...'" />
          <IconCross @click="deleteSubtask(index)" class="cursor-pointer" />
        </div>
        <ButtonSecondaryLarge type="button" @click.stop="addSubtask">+ YENI ALT IS PAKETI EKLE</ButtonSecondaryLarge>
      </div>
